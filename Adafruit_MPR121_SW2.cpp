...
boolean Adafruit_MPR121_SW2::begin(uint8_t i2caddr) {
  ...
  // soft reset
  writeRegister(MPR121_SW2_SOFTRESET, 0x63);
  delay(1);
  ...
  writeRegister(MPR121_SW2_ECR, 0x0);

  uint8_t c = readRegister8(MPR121_SW2_CONFIG2);

  if (c != 0x24) return false;


  setThreshholds(12, 6);
  writeRegister(MPR121_SW2_MHDR, 0x01);
  writeRegister(MPR121_SW2_NHDR, 0x01);
  writeRegister(MPR121_SW2_NCLR, 0x0E);
  writeRegister(MPR121_SW2_FDLR, 0x00);

  writeRegister(MPR121_SW2_MHDF, 0x01);
  writeRegister(MPR121_SW2_NHDF, 0x05);
  writeRegister(MPR121_SW2_NCLF, 0x01);
  writeRegister(MPR121_SW2_FDLF, 0x00);

  writeRegister(MPR121_SW2_NHDT, 0x00);
  writeRegister(MPR121_SW2_NCLT, 0x00);
  writeRegister(MPR121_SW2_FDLT, 0x00);

  writeRegister(MPR121_SW2_DEBOUNCE, 0);
  writeRegister(MPR121_SW2_CONFIG1, 0x10); // default, 16uA charge current
  writeRegister(MPR121_SW2_CONFIG2, 0x20); // 0.5uS encoding, 1ms period

//  writeRegister(MPR121_SW2_AUTOCONFIG0, 0x8F);

//  writeRegister(MPR121_SW2_UPLIMIT, 150);
//  writeRegister(MPR121_SW2_TARGETLIMIT, 100); // should be ~400 (100 shifted)
//  writeRegister(MPR121_SW2_LOWLIMIT, 50);
  // enable all electrodes
  writeRegister(MPR121_SW2_ECR, 0x8F);  // start with first 5 bits of baseline tracking
 ...
 ****************************************************************************************/
void Adafruit_MPR121_SW2::setThreshholds(uint8_t touch, uint8_t release) {
 ...
 ****************************************************************************************/
void Adafruit_MPR121_SW2::setThresholds(uint8_t touch, uint8_t release) {
  for (uint8_t i=0; i<12; i++) {
    writeRegister(MPR121_SW2_TOUCHTH_0 + 2*i, touch);
    writeRegister(MPR121_SW2_RELEASETH_0 + 2*i, release);
  }
}
 ...
 ****************************************************************************************/
uint16_t  Adafruit_MPR121_SW2::filteredData(uint8_t t) {
  if (t > 12) return 0;
  return readRegister16(MPR121_SW2_FILTDATA_0L + t*2);
}
 ...
 ****************************************************************************************/
uint16_t  Adafruit_MPR121_SW2::baselineData(uint8_t t) {
  if (t > 12) return 0;
  uint16_t bl = readRegister8(MPR121_SW2_BASELINE_0 + t);
  return (bl << 2);
}
 ...
 ****************************************************************************************/
uint16_t  Adafruit_MPR121_SW2::touched(void) {
  uint16_t t = readRegister16(MPR121_SW2_TOUCHSTATUS_L);
  return t & 0x0FFF;
}
